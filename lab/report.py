...
def write(recs, out):
    ...
    for row in [["Chat sheet", "no-documents benchmark: the context to reduce is the conversation itself (lab/corpus_chat.py). chat_full = whole transcript in context; chat_window = last 10 exchanges; chat_summary = rolling LLM summary (compaction) + last 10; chat_repl = transcript in a Python variable + last 10; chat_state = online fact ledger (one extraction call per exchange) in the prompt + last 10; chat_vc = ledger + tools (search over facts, search over raw exchanges, open_turn, python)"],
                ...
                ["Chat sheet (wild conditions, tag wild/wild2/wildfix/wildaudit)", "no raw window in any of them: chat_ledger0 = ledger only; chat_terse = ledger as one line per entity (attr=value, history in brackets); chat_slice = hybrid top-12 ledger entries for the question + every entry whose attribute is named in the question + entity directory; chat_tools0 = tools only, nothing in the prompt; chat_shorthand = one LLM call compresses the whole transcript into notes for itself; chat_rag = single-shot top-6 raw exchanges; chat_useronly = user turns only, no LLM; chat_asstclip = user turns + first sentence of each reply, no LLM; chat_regex = user sentences with a number/identifier/name, no LLM; chat_prune = user turns with stop-words removed, no LLM. Tag wildk4 = chat_slice with top-4. wildref = chat_state/chat_summary on the messy long and dense chats"],
